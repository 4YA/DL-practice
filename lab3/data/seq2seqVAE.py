...
class VAE(nn.Module):
    ...
    def forward(self, input_tensor, target_tensor, encoder_hidden, c):
        ...
        decoder_cn = hidden[1][-1].unsqueeze(0)
    ...
    def initEncoderHidden(self):
        return torch.zeros(1 , 1, self.encoder_hidden_size, device=device)
    def initDecoderHidden(self):
...
def Gaussian_test(CVAE):
    ...
    for t in range(100):
        ...
        latent_c = torch.randn(hidden_size).to(device)
        for i in range(4):
...
def train(input_tensor, target_tensor, CVAE, CVAE_optimizer, criterion,label,KLD_weight):
    ...
    CVAE.zero_grad()
    encoder_hidden = CVAE.initEncoderHidden()
    loss = 0
    
    target_length = target_tensor.size(0)
    outputs, means, log_vars = CVAE(input_tensor.to(device), target_tensor.to(device), encoder_hidden, label)
   
    crossEntropyLoss = 0
    for i in range(target_length):
    
        crossEntropyLoss += criterion(outputs[i], target_tensor[i].unsqueeze(0).to(device))
...
def test(input_tensor, target_tensor, CVAE):
    ...
    outputs, means, log_vars = CVAE(input_tensor[0].to(device), target_tensor[0].to(device), encoder_hidden, onehot)
...
if __name__ == '__main__':
    ...
    if args.mode == 0:
        ...
        trainIters(CVAE,500000, print_every=1000, save_every = 50000,KL=0)
