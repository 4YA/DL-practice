 ...
 * use 'g++ -std=c++11 -O3 -o 2048 2048.cpp' to compile the source
 ...
 * Technologies and Applications of Artificial Intelligence. Springer
 * International Publishing, 2014. 366-378.
 *
 * [3] Oka, Kazuto, and Kiminori Matsuzaki. "Systematic selection of n-tuple
 * networks for 2048." International Conference on Computers and Games. Springer
    ...
    for (state *move = after; move != after + 4; move++) {
      if (move->assign(b)) {
        ...
        for(int i=0;i<16;i++){
          ...
          if(temp.at(i) == 0){
              temp.set(i,1);
              value += estimate(temp) * 0.9;
              temp = move->after_state();
              temp.set(i,2);
              value += estimate(temp) * 0.1;
              total+=1;
          }
        }
        move->set_value(move->reward() + value / total);
